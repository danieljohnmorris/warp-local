...
- **Local-model selector**. When `WARP_BYPASS_AUTH=1` is set, the model-selector menu (footer button) replaces Warp's server-fetched model list with a local set: Claude Sonnet 4.7, Opus 4.7, Haiku 4.5; GPT-5.5 at low/medium/high reasoning effort; and the Ollama section (see below).
...
- **User menu: What's new, Feedback, Slack, Upgrade hidden under bypass**. The four cloud-dependent user-menu entries are suppressed when `WARP_BYPASS_AUTH=1`: "What's new" (cloud changelog viewer), "Feedback" (cloud feedback form), "Slack" (community invite), and "Upgrade" (cloud subscription upsell). Settings, Keyboard shortcuts, Documentation, View Warp logs, and Log out remain.
